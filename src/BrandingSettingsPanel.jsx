...
function UploadButton({ accept, onDataUrl, label: btnLabel }) {
  const ref = useRef();
...
export default function BrandingSettingsPanel({ onClose, showToast }) {
  ...
  return (
    <div style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <div style={{ gridColumn: "1/-1" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              {form.logoUrl && <button type="button" onClick={() => up("logoUrl", "")} style={{ padding: "8px 10px", borderRadius: 8, border: "1px solid #fecaca", background: "#fef2f2", fontSize: 12, color: "#ef4444", cursor: "pointer", fontFamily: "inherit", flexShrink: 0 }}>✕</button>}
            </div>
            {form.logoUrl && (
          ...
          <div style={{ gridColumn: "1/-1" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              {form.faviconUrl && <button type="button" onClick={() => up("faviconUrl", "")} style={{ padding: "8px 10px", borderRadius: 8, border: "1px solid #fecaca", background: "#fef2f2", fontSize: 12, color: "#ef4444", cursor: "pointer", fontFamily: "inherit", flexShrink: 0 }}>✕</button>}
            </div>
            {form.faviconUrl && (
              <div style={{ marginTop: 10, display: "inline-flex", alignItems: "center", gap: 8 }}>
                <img src={form.faviconUrl} alt="Favicon preview" style={{ width: 24, height: 24, objectFit: "contain", border: "1px solid #e2e8f0", borderRadius: 4, background: "#fff", padding: 2 }} onError={e => e.target.style.display = "none"} />
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 10, border: `2px solid ${form.sidebarDark === opt.val ? form.primaryColor : "#e2e8f0"}`,
                background: opt.bg, cursor: "pointer", fontFamily: "inherit",
              }}>
              <div style={{ width: 32, height: 48, borderRadius: 6, background: opt.bg, border: "1px solid rgba(0,0,0,0.1)", display: "flex", flexDirection: "column", padding: 4, gap: 3 }}>
                {[1,2,3].map(i => <div key={i} style={{ height: 6, borderRadius: 3, background: opt.val ? "rgba(255,255,255,0.25)" : "rgba(0,0,0,0.15)" }} />)}
              </div>
      ...
      {/* Live preview chip */}
      <div style={{ background: "#f8fafc", borderRadius: 10, padding: "14px 18px", marginBottom: 20, display: "flex", alignItems: "center", gap: 14 }}>
        <span style={{ fontSize: 12, color: "#64748b", fontWeight: 600 }}>Preview:</span>
        <div style={{ background: form.primaryColor, color: "#fff", borderRadius: 8, padding: "6px 14px", fontSize: 12, fontWeight: 700 }}>Primary button</div>
        <div style={{ background: form.accentColor, color: "#fff", borderRadius: 8, padding: "6px 14px", fontSize: 12, fontWeight: 700 }}>Accent button</div>

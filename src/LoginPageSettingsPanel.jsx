...
export default function LoginPageSettingsPanel({ onClose, showToast }) {
  ...
  useEffect(() => {
    getLoginBranding()
      .then(setForm)
      .catch(() => showToast?.("Failed to load Login Page settings.", "err"))
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const saved = await updateLoginBranding(form);
      setForm(saved);
      setSaved(true);
      setTimeout(() => setSaved(false), 2000);
    } catch {
      showToast?.("Failed to save Login Page settings.", "err");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#0f172a" }}>Login Page Settings</h2>
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "#64748b" }}>Customize the staff sign-in screen. Independent from the app's Appearance settings.</p>
        </div>
      ...
      <section style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <div>
            {label("Subtitle")}
            <input style={inp} value={form.subtitle} onChange={e => up("subtitle", e.target.value)} placeholder="internal use only" />
          </div>

...
- **Office UI toggle** — a topbar button lets any user switch the whole app (sidebar, topbar, and every page) between the default Modern look and a Microsoft/Office 365-style theme matching the Login Page (Segoe UI, neutral `#F3F2F1` canvas, sharp corners, flat Fluent-style sidebar). This is a personal per-browser display preference (saved to `localStorage`), not shared company branding — it doesn't affect what other users see, and it layers on top of the admin-configured primary/accent colors rather than replacing them

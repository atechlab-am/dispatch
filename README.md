...
- **Office UI toggle** — a topbar button lets any user switch the whole app (sidebar, topbar, and every page) between the default Modern look and a Microsoft/Office 365-style theme matching the Login Page (Segoe UI, neutral `#F3F2F1` canvas, sharp corners everywhere including cards/buttons/inputs, flat Fluent-style sidebar). This is a personal per-browser display preference (saved to `localStorage`), not shared company branding — it doesn't affect what other users see, and it layers on top of the admin-configured primary/accent colors rather than replacing them. Status pills, badges, dots, and avatars stay round in both themes, matching their usual shape. Both themes' entire look — colors, font, and all three corner-radius sizes — are centrally defined in `src/theme.css`, so restyling either theme going forward means editing that one file, not hunting through every page component

...
class DocumentBranding(Base):
    """Quote/Invoice PDF + email appearance — a single row (id=1), admin-managed.
    Applies to both quote and invoice documents (one company, one look), unlike
    Branding/LoginPageBranding/PortalBranding which each cover a distinct screen.
    Never publicly readable — PDFs/emails are always generated by an authenticated
    staff request, never rendered client-side before a session exists.

    Two layers of customization: structured font-size options (always applied
    to the built-in layout) and an optional raw-HTML custom template per
    document type, which — when enabled — completely replaces the built-in
    layout for that document type. See app/document_branding.py."""
    ...
    # Structured font-size controls (px), applied to the built-in layout.
    font_size_header = Column(Integer, nullable=False, default=22)
    font_size_body = Column(Integer, nullable=False, default=14)
    font_size_table = Column(Integer, nullable=False, default=13)
    font_size_totals = Column(Integer, nullable=False, default=15)

    # Custom raw-HTML templates (nullable — empty means "use the built-in
    # layout"). Independently toggleable per document type.
    use_custom_invoice_template = Column(Boolean, nullable=False, default=False)
    custom_invoice_template = Column(Text, nullable=False, default="")
    use_custom_quote_template = Column(Boolean, nullable=False, default=False)
    custom_quote_template = Column(Text, nullable=False, default="")

...
Three reports, all admin-only, all with optional date-range filtering:
  GET /reports/revenue   — total billed per month, breakdown by client and service type
  GET /reports/technician — tickets resolved and hours logged per technician
  GET /reports/sla        — SLA compliance % per priority over a date range

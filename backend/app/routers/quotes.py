...
def _quote_template_context(q: Quote, branding) -> dict:
    """Build the full {{placeholder}} -> value context for a quote document.
    Shared by the default built-in layout and any custom template."""
    ...
    return {
        ...
        "client_name": html_lib.escape(q.client_name or "—"),
        "client_email_html": f"<p>{html_lib.escape(q.client_email)}</p>" if q.client_email else "",
        "project_html": f"<p><strong>Project:</strong> {html_lib.escape(q.project_name)}</p>" if q.project_name else "",
        ...
        "subtotal": f"{float(q.subtotal):,.2f}",
        "tax_line_html": "" if tax_pct == 0 else f"<tr><td>Tax ({tax_pct}%)</td><td style='text-align:right'>${float(q.tax_amount):,.2f}</td></tr>",
        "total": f"{float(q.total):,.2f}",
        "notes_html": f'<div class="notes">{html_lib.escape(q.notes)}</div>' if q.notes else "",
    }


_QUOTE_DEFAULT_TEMPLATE = """<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<div class="page">
  <div class="header">
    ...
    <div style="text-align:right">
      <div style="font-size:28px;font-weight:800;letter-spacing:-0.5px;opacity:0.95">{{quote_id}}</div>
  ...
  <div class="footer">{{company_name}} &nbsp;|&nbsp; {{website}}</div>
</div>
...
def _build_quote_html(q: Quote, db: Session) -> str:
    ...
    context = _quote_template_context(q, branding)
    if branding.use_custom_quote_template and branding.custom_quote_template:
        try:
            return render_template(branding.custom_quote_template, context)
        except TemplateRenderError:
            pass
    return render_template(_QUOTE_DEFAULT_TEMPLATE, context)

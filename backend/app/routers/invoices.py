...
def _invoice_template_context(inv: Invoice, branding) -> dict:
    ...
    if inv.payments:
        rows = "".join(
            ...
            f"<td style='padding:6px 12px;color:{branding.muted_color}'>{html_lib.escape(p.note)}</td></tr>"
            for p in inv.payments
        )
        payments_html = f"""
        <h3 style='margin:28px 0 10px;font-size:13px;text-transform:uppercase;letter-spacing:0.5px;color:{branding.muted_color}'>Payments Received</h3>
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:'Inter',Arial,sans-serif;font-size:{{font_size_body}}px;color:{{text_color}};background:#f1f5f9;padding:32px}
